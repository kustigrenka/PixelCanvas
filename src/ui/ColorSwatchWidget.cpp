...
// ─────────────────────────────────────────────────────────────────────────────
// Column count  –  recomputed on resize
// ─────────────────────────────────────────────────────────────────────────────
...
void ColorSwatchWidget::paintEvent(QPaintEvent *)
{
    ...
    for (int i = 0; i < kCount; ++i)
    {
        ...
        if (!c.isValid())
        {
            // Checkerboard pattern for empty slots.
            constexpr int sq = 4;
...
void ColorSwatchWidget::mousePressEvent(QMouseEvent *e)
{
    ...
    if (e->button() == Qt::LeftButton)
    {
        ...
        {
            // Empty slot — left-click saves the current colour.
        ...
        if (idx >= 0)
        {
            ...
            QToolTip::showText(he->globalPos(),
                c.isValid() ? c.name().toUpper()
                            : tr("Empty — click to save current color, "
