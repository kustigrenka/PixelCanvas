...
//
// FIX 3: resizeEvent no longer wipes m_canvas.  Content is composited onto
// a fresh background only when the widget actually grows, preserving strokes
// across accordion open/close animations (which generate many resize events
// as maximumHeight is animated from 0 to the natural height).
// ─────────────────────────────────────────────────────────────────────────────
...
private:
    ...
    QImage        m_canvas;
    QSize         m_canvasSize;          // size the canvas was last allocated at
    BrushEngine  *m_engine      = nullptr;
    QColor        m_color       = Qt::black;
    QImage       *m_savedLayer  = nullptr;
