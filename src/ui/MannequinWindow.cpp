...
static constexpr char kPinterestUrl[] = "https://posemy.art/";
...
void MannequinWindow::buildUI(const QString &url)
{
    ...
    titleBar->setStyleSheet("QWidget { background:#1a5276; border-bottom:1px solid #0d2840; }");
    ...
    pinIcon->setStyleSheet("QLabel { color:#9ab8e8; font-size:16px; background:transparent; border:none; }");
    ...
    connect(homeBtn, &QToolButton::clicked, this, [this]() {
        navigate("https://posemy.art/");
    });
    navLayout->addWidget(homeBtn);
#endif

    m_urlBar = new QLineEdit(url.isEmpty() ? QString(kPinterestUrl) : url, navBar);
    ...
    m_webView->setUrl(QUrl(url.isEmpty() ? QString(kPinterestUrl) : url));

    // Handle "Open in new tab/window" — load in same view instead
    ...
    auto *desc = new QLabel(
        "Qt WebEngine is not available.\nClick below to open Pinterest in your browser.",
        fallbackWidget);
    ...
    openBtn->setText("Open pinterest.com  ↗");
