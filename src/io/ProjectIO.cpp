...
//
// Entries are stored DEFLATED (zlib deflate, wbits = -15).
// ─────────────────────────────────────────────────────────────────────────────
...
QByteArray deflateRaw(const QByteArray &src)
{
    ...
    QByteArray out;
    out.resize(static_cast<int>(deflateBound(&zs, static_cast<uLong>(src.size()))));
...
struct ZipReader
{
    ...
    // Scan from the end of the buffer for the EOCD signature.
    ...
    QByteArray extractFile(const QByteArray &name) const
    {
        ...
        for (int i = 0; i < numFiles; ++i)
        {
            ...
            const QByteArray entryName = data.mid(pos + 46, nameLen);
            ...
            const QByteArray compData = data.mid(dataStart, static_cast<int>(compSz));

            if (method == 0) return compData;   // stored (uncompressed)
...
bool ProjectIO::loadProject(const QString &path)
{
    ...
    if (layerArray.isEmpty()) return false;
    ...
    for (const QJsonValue &val : layerArray)
    {
        ...
        QImage img;
        if (!img.loadFromData(pngBytes, "PNG")) return false;
